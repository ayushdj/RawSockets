...
class MyRawSocket:
    ...
    def perform_handshake(self, source_ip_address, source_port, destination_ip_address):
        ...
        return self._receive_synack_from_server(
            source_ip_address, source_port, destination_ip_address
        )

    def request_for_and_download_resource(
        self,
        source_ip_address,
        source_port,
        ...
        self._request_for_resource_in_server(
            source_ip_address,
            source_port,
            destination_ip_address,
            self.determine_url_host(url),
            path_to_file,
            unpacked_tcp_header_from_server,
        )

        self._get_packets_and_create_file(
            name_of_file, source_ip_address, source_port, destination_ip_address
        )

    def _send_syn_to_server(
        self, source_ip_address, source_port, destination_ip_address
    ) -> None:
        """
        ...
        self._send_a_packet_to_server(
            construct_IPV4_header(42069, source_ip_address, destination_ip_address)
            + develop_TCP_header(
                self.syn_flags,
                source_port,
                0,
    ...
    def _receive_synack_from_server(
        self, source_ip_address, source_port, destination_ip_address
    ):
        """
        ...
        (
            unpacked_ip_header_from_server,
            unpacked_tcp_header_from_server,
            _,
            _,
        ) = self._extract_packet_data_from_server(packet_from_server)

        # get the source and destination IP's from the server
        (
            source_ip_address_from_server,
            destination_ip_address_from_server,
        ) = self._extract_source_and_dest_ip_from_server(unpacked_ip_header_from_server)
        ...
        if not (
            ...
            and source_ip_address_from_server == destination_ip_address
            and unpacked_tcp_header_from_server[5] == 18
            and destination_ip_address_from_server == source_ip_address
            and source_port == unpacked_tcp_header_from_server[1]
        ):
            self._send_syn_to_server(
                source_ip_address, source_port, destination_ip_address
            )
        else:
    ...
    def _send_acknowledgement(
        ...
        self._send_a_packet_to_server(
            construct_IPV4_header(42070, source_ip_address, destination_ip_address)
            + develop_TCP_header(
                self.acknowledgement_flags,
                source_port,
                unpacked_tcp_header_from_server[3],
                unpacked_tcp_header_from_server[2] + 1,
                source_ip_address,
                destination_ip_address,
            ),
            destination_ip_address=destination_ip_address,
    ...
    def _request_for_resource_in_server(
        self,
        source_ip_address,
        source_port,
        destination_ip_address,
        ...
        self._send_a_packet_to_server(
            construct_IPV4_header(42071, source_ip_address, destination_ip_address)
            + develop_TCP_header(
                self.resource_request_flags,
                source_port,
                unpacked_tcp_header_from_server[3],
                unpacked_tcp_header_from_server[2] + 1,
                source_ip_address,
    ...
    def _send_a_packet_to_server(
        self, packet_to_be_sent, destination_ip_address
    ) -> None:
        """
    ...
    def _get_packets_and_create_file(
        self, name_of_file, source_ip_address, source_port, destination_ip_address
    ):
        """
        ...
        while 1:
            ...
            (
                unpacked_ip_header_from_server,
                unpacked_tcp_header_from_server,
                actual_ip_header_length,
                actual_tcp_header_length,
            ) = self._extract_packet_data_from_server(packet_from_server)

            # actual_size_of_data = abs((actual_ip_header_length + actual_tcp_header_length * 4) - len(packet_from_server))

            if (
                unpacked_tcp_header_from_server[1] == source_port
                and socket.inet_ntoa(unpacked_ip_header_from_server[8])
                == destination_ip_address
            ):
                # if we have more data to receive from the server, then we keep constructing the result
                if (
                    abs(
                        (actual_ip_header_length + actual_tcp_header_length * 4)
                        - len(packet_from_server)
                    )
                    > 0
                ):
                    # populate the data
                    data_from_server[
                        unpacked_tcp_header_from_server[2]
                    ] = packet_from_server[
                        (actual_ip_header_length + actual_tcp_header_length * 4) :
                    ]
                    ...
                    self._send_a_packet_to_server(
                        construct_IPV4_header(
                            42070, source_ip_address, destination_ip_address
                        )
                        + develop_TCP_header(
                            self.acknowledgement_flags,
                            source_port,
    ...
    def _extract_packet_data_from_server(self, packet_from_server):
        ...
        unpacked_ip_header_from_server = struct.unpack(
            "!BBHHHBBH4s4s", packet_from_server[:20]
        )
        (
        ...
        tcp_header_from_server = packet_from_server[
            actual_ip_header_length : actual_ip_header_length + 20
        ]
        unpacked_tcp_header_from_server = struct.unpack(
            "!HHLLBBHHH", tcp_header_from_server
        )
    ...
    def _extract_source_and_dest_ip_from_server(self, unpacked_ip_header_from_server):
        ...
        return socket.inet_ntoa(source_ip_address_from_server), socket.inet_ntoa(
            destination_ip_address_from_server
        )
