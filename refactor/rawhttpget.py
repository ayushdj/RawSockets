...
def write_to_file(file_name, res):
    ...
    if not http_response.startswith(bytearray("HTTP/1.1 200 OK", "utf-8")):
        ...
        file_pointer.write(
            res[proper_sequence[0]].split(bytearray("\r\n\r\n", "utf-8"))[1]
        )
...
#####################################################################
def get_synack_send_ack(
    send_sock, recv_sock, buffer_size, source_ip, dest_ip, src_port
):
...
#####################################################################
def send_request_for_file(
    send_sock, source_ip, dest_ip, src_port, tcph, path_url, hostname
):
    # send a HTTP Get
    httprequest_packet = ""
    ...
    tcp_header = create_tcp_header(
        tcp_source, tcp_seq, tcp_ack_seq, tcp_fin, tcp_syn, tcp_rst, tcp_psh, tcp_ack
    )
    ...
    httprequest_packet = ip_header + tcp_header + bytes(request_httpdata, "utf-8")
...
def download_file(send_sock, recv_sock, buffer_size, source_ip, dest_ip, src_port, fp):
    ...
    while True:
        ...
        if dest_port == src_port and src_addr == dest_ip and data_size > 0:
            ...
            # final full packet - syn packets dont have any data
            teardown_initiator = (
                ip_header + tcp_header + bytes(data_for_teardown, "utf-8")
            )
        ...
        if (
            ...
            # final full packet - syn packets dont have any data
            fin_packet = ip_header + tcp_header + bytes(data_in_finpacket, "utf-8")
...
def start():
    ...
    split_url = urlsplit(url_parameter)
    hostname = split_url.netloc
    # storing sourceIP
    source_ip = ip_of_localhost()
    # storing destinationIP
    dest_ip = socket.gethostbyname(urlparse(url_parameter).hostname)
